...
def build_project(project_config):
    ...
    if project_config.get('cmd_build'):
        ...
        if code != 0:
            raise Exception("cmd build失败，code：{}".format(code))
        return
        
    # cmake
    code = subprocess.call(cmake_options, cwd=build_path)
    if code != 0:
        raise Exception("cmake失败，code：{}".format(code))
    # build
    code = subprocess.call(build_options, cwd=build_path)
    if code != 0:
        raise Exception("build失败，code：{}".format( code))
    # install
    code = subprocess.call(install_options, cwd=build_path)
    if code != 0:
        raise Exception("install失败，code：{}".format(code))